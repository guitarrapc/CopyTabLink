...
- キーボードショートカットでコピー (`Alt+C`/`Option+C`, `Alt+M` / `Option+M`)
- コンテキストメニューからコピー
...
- Copy with keyboard shortcuts (`Alt+C`/`Option+C`, `Alt+M` / `Option+M`)
...
- Copia con atajos de teclado (`Alt+C`/`Option+C`, `Alt+M` / `Option+M`)
...
- Copie avec des raccourcis clavier (`Alt+C`/`Option+C`, `Alt+M` / `Option+M`)
...
- 透過鍵盤快速鍵複製 (`Alt+C`/`Option+C`, `Alt+M` / `Option+M`)
- 從右鍵選單複製
...
- 支持使用键盘快捷键复制 (`Alt+C`/`Option+C`, `Alt+M` / `Option+M`)
- 支持从右键菜单复制
