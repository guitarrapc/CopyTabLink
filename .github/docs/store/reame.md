...
CopyTabLink は、現在開いているページのタイトルと URL をすばやくコピーする Chrome 拡張です。

- キーボードショートカットでコピー (`Alt+C` (Windows/Linux) / `Option+C` (Mac))
- コンテキストメニューからコピー
...
CopyTabLink helps you quickly copy the current page title and URL.

- Copy with keyboard shortcuts (`Alt+C` (Windows/Linux) / `Option+C` (Mac))
...
CopyTabLink te ayuda a copiar rapidamente el titulo y la URL de la pagina actual.

- Copia con atajos de teclado (`Alt+C` (Windows/Linux) / `Option+C` (Mac))
...
CopyTabLink vous aide a copier rapidement le titre et l'URL de la page en cours.

- Copie avec des raccourcis clavier (`Alt+C` (Windows/Linux) / `Option+C` (Mac))
...
CopyTabLink 可讓你快速複製目前頁面的標題與網址。

- 透過鍵盤快速鍵複製 (`Alt+C` (Windows/Linux) / `Option+C` (Mac))
- 從右鍵選單複製
...
CopyTabLink 可帮助你快速复制当前页面的标题和链接。

- 支持使用键盘快捷键复制 (`Alt+C` (Windows/Linux) / `Option+C` (Mac))
- 支持从右键菜单复制
